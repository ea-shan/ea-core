...
function express_analytics_add_cors_headers()
{
	...
	if (
		...
		header('Access-Control-Allow-Origin: https://localhost:10019/');
		header('Access-Control-Allow-Methods: GET, POST, OPTIONS');
		header('Access-Control-Allow-Headers: Content-Type');
	}
}
add_action('send_headers', 'express_analytics_add_cors_headers');
...
// Ensure 'page-attributes' support for built-in post types (page and post)
...
add_action('wp_footer', function () {
	if (is_singular() && $form_code = get_field('hubspot_form_code')) {
?>
		<script src="//js.hsforms.net/forms/v2.js"></script>
		<script>
			document.addEventListener("DOMContentLoaded", function() {
				<?php echo html_entity_decode($form_code); ?>
			});
		</script>
<?php
	}
...
add_action('wp_footer', function () {
	echo '<script src="' . get_template_directory_uri() . '/assets/js/cookie-consent.js"></script>';
});

// Add Vapi chat widget to the footer, scoped to avoid CSS interference
